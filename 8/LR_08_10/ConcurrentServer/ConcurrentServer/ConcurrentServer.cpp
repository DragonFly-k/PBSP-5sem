...
bool AcceptCycle(int sq)	//подключение клиентов и добавление в List
{							
	bool rc = false;
	Contact c(Contact::ACCEPT, "EchoServer");
	while (sq-- > 0 && !rc)//цикл обработки комманд
	{
		...
		{
			rc = true;               // подключился
			EnterCriticalSection(&scListContact);//вход в критическую секцию
			contacts.push_front(c);
			LeaveCriticalSection(&scListContact);//покинуть секцию
			puts("contact connected");
			InterlockedIncrement(&connectionCount);//+1 к значению крит. секций
		}
	}
	return rc;
};
...
void openSocket() {
	...
	if (ioctlsocket(sS, FIONBIO, &nonblk) == SOCKET_ERROR)
		//переключение режимов сокета (blocking/nonblocking)
...
DWORD WINAPI AcceptServer(LPVOID pPrm)    // прототип (сервер ожидания запроса)
{
	...
	try
	{
		...
		CommandsCycle(*((TalkersCommand*)pPrm));//цикл обработки комманд
...
DWORD WINAPI ConsolePipe(LPVOID pPrm)    // прототип (серверная часть консоли управления)
{
	DWORD rc = 0;
	char rbuf[100];
		...
		while (param != EXIT) {
			...
			while (ReadFile(hPipe, rbuf, sizeof(rbuf), &dwRead, NULL))
			{
				...
				if (param == LOAD_LIB)
				{
					...
					EnterCriticalSection(&scListContact);//вход в крит. точку
...
DWORD WINAPI GarbageCleaner(LPVOID pPrm)    // прототип (удаление клиента из списка подлкючений)
{
	DWORD rc = 0;    // код возврата 	
...
void CALLBACK ASWTimer(LPVOID Prm, DWORD, DWORD) {	//ожидающий таймер для подключений
...
bool PutAnswerToClient(char* name, sockaddr* to, int* lto) {
	//подтверждение сервером запроса клиента на установку соединения
